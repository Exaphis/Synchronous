...
class ChangePasswordView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, unique_id=None, **kwargs):
        if unique_id is None:
            return Response(
                {"error": True, "detail": "No unique ID."},
        ...
        if workspace.user is None:
            return Response(
                {"error": True, "detail": "Workspace has no password."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        with import_zip.open("workspace.json") as workspace_info_file:
            ...
            for import_tab in workspace_info:
                tab = WorkspaceTab.objects.create(
                    workspace=workspace, name="Untitled tab"
                )
                for app in import_tab:
                    app_type = app["type"]
                    if app_type == AppType.PAD:
                        pad = WorkspacePadApp.objects.create_pad(
                            tab=tab, name="Text pad"
                        )
                        with import_zip.open(app["contents"]) as contents:
                            pad.import_data(contents.read())
                    elif app_type == AppType.WHITEBOARD:
                        board = WorkspaceWhiteboardApp.objects.create_whiteboard(
                            tab=tab, name="Whiteboard"
                        )
...
class CustomAuthToken(ObtainAuthToken):
    def post(self, request, *args, **kwargs):
        ...
        if "unique_id" not in data:
            raise ValidationError('Must include "unique_id".', code="authorization")

        try:
            workspace = Workspace.objects.get(unique_id=data["unique_id"])
        except ObjectDoesNotExist:
            raise ValidationError(
                'Workspace with given "unique_id" does not exist.', code="authorization"
            )

        if workspace.user is None:
            raise ValidationError(
                "Workspace is globally editable", code="authorization"
            )
        ...
        return Response({"token": token.key, "unique_id": user.workspace.unique_id})
